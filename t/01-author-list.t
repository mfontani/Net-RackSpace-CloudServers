...
SKIP: {
  skip 'more than one server found, author has only one', 9 if ( scalar @current_servers != 1 );
  ...
  note 'Public address: ', join( ' ', @{ $current_servers[0]->public_address || () } );
  ok( defined $current_servers[0]->public_address, 'has defined public address' );
  is @{ $current_servers[0]->public_address }, 1, 'has 1 defined public address';
  ok( $current_servers[0]->public_address->[0] =~ /^98\.129\.236/, 'has right public address' );
  note 'Private address: ', join( ' ', @{ $current_servers[0]->private_address || () } );
  ok( defined $current_servers[0]->private_address, 'has defined private address' );
  is @{ $current_servers[0]->private_address }, 1, 'has 1 defined private address';
  ok( $current_servers[0]->private_address->[0] =~ /^10\.176\.135/, 'has right private address' );
}
